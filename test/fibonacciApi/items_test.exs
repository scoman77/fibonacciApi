defmodule FibonacciApi.ItemsTest do
  ...
  describe "numbers" do
    ...
    test "create_number/1 with valid data creates a number" do
      valid_attrs = %{blacklisted: true, description: "some description", value: 42}

      assert {:ok, %Number{} = number} = Items.create_number(valid_attrs)
      assert number.blacklisted == true
      assert number.description == "some description"
    ...
    test "update_number/2 with valid data updates the number" do
      number = number_fixture()
      update_attrs = %{blacklisted: false, description: "some updated description", value: 43}

      assert {:ok, %Number{} = number} = Items.update_number(number, update_attrs)
      assert number.blacklisted == false
      assert number.description == "some updated description"
